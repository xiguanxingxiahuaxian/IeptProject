 ...
 * 修改备注
 */
public class RemessageAdapter extends SuperBaseAdapter<messageDTO.PageBean.ListBean>{

    public RemessageAdapter(Context context, List<messageDTO.PageBean.ListBean> data) {
        super(context, data);
    }
    ...
    protected void convert(BaseViewHolder holder, messageDTO.PageBean.ListBean item, int position) {
        ...
        holder.setText(R.id.fy,item.getReply()==null?"":item.getReply());
    }

    @Override
    protected int getItemViewLayoutId(int position, messageDTO.PageBean.ListBean item) {
        return R.layout.remessage_item_layout;
    }
}
